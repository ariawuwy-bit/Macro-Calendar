...
# Old July focus block (exact from file)
old_focus = """  focus: [
...
if old_focus in content:
    print("Found old focus block!")
else:
    print("Old focus NOT found, searching...")
    # Try to find with flexible whitespace
    pattern = re.compile(r'focus:\s*\[.*?BOJ政策.*?\}\s*\]', re.DOTALL)
    match = pattern.search(content)
    if match:
        print("Found via regex!")
        print("Matched length:", len(match.group()))
    else:
        print("Not found via regex either")
        # Find the exact location of 'BOJ政策' in July section
        idx = content.find('BOJ政策')
        while idx != -1:
            # Check if it's in July (after JULY marker)
            if content.rfind('// ===== JULY 2026 =====', 0, idx) > 0:
                print(f"Found 'BOJ政策' at {idx}")
                # Print context
                print("Context:", repr(content[max(0,idx-100):idx+200]))
                break
            idx = content.find('BOJ政策', idx+1)
